...
func _process(_delta):
	...
	if ($Boss/EnemyActor):
		ui.get_node("BossHealth").size.x = float(boss.cur_health)/boss.max_health*boss_health_max_x_size
	else:
		ui.get_node("BossHealth").size.x = 0
...
func _on_StonesArea_body_entered(body):
	if body.is_in_group("players"):
		...
		body.connect("action_use",Callable(self,"_pickup_stones"))
...
func _on_StonesArea_body_exited(body):
	if body.is_in_group("players"):
		...
		body.disconnect("action_use",Callable(self,"_pickup_stones"))
